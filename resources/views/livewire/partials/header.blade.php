            ...
            <div class="flex justify-between items-center h-14 sm:h-16">
                    ...
                    <a href="{{ route('home') }}" class="flex items-center space-x-2 sm:space-x-3 group">
                        ...
                        @else
                            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-orange-500 to-purple-500 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-200 group-hover:scale-105">
                                <span class="text-white text-sm sm:text-base font-semibold">{{ substr($siteName, 0, 1) }}</span>
                            </div>
                        @endif
                        <span class="text-lg sm:text-xl font-semibold bg-gradient-to-r from-orange-600 to-purple-600 bg-clip-text text-transparent hidden sm:block">{{ $logoText }}</span>
                    </a>
                </div>
                ...
                <div class="flex items-center space-x-2 sm:space-x-3">
                    ...
                    <div class="hidden lg:flex items-center space-x-2 bg-gradient-to-r from-orange-50 to-purple-50 px-3 py-1.5 rounded-lg border border-orange-200">
                        ...
                        <span class="text-sm font-semibold text-gray-700">{{ number_format(auth()->user()->credits ?? 0) }}</span>
                    </div>
                    ...
                    <div class="relative" x-data="{ open: false }" @keydown.escape.window="open = false">
                        ...
                        <div x-show="open"
                            ...
                            <div class="px-4 py-3 bg-gradient-to-r from-orange-50 to-purple-50">
                                <div class="grid grid-cols-2 gap-3 text-center">
                                    <div>
                                        ...
                                        <p class="text-sm font-semibold text-orange-600">{{ number_format(auth()->user()->credits ?? 0) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-600">Balance</p>
                                        <p class="text-sm font-semibold text-purple-600">₦{{ number_format(auth()->user()->wallet_balance ?? 0, 2) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="py-2 border-b border-gray-100 lg:hidden">
                                <a href="{{ route('home') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('home') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                ...
                                <a href="{{ route('channels.index') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('channels.*') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                    <svg class="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.785"/></svg>
                                    Channels
                                </a>
                                <a href="{{ route('channel-bookings.index') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('channel-bookings.*') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                                    Channel Bookings
                                </a>
                                <a href="{{ route('ads.index') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('ads.*') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1"></path></svg>
                                    Share & Earn
                                </a>
                                <a href="{{ route('my-batches') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('my-batches') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                                    My Batches
                                </a>
                                <a href="{{ route('credits.purchase') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('credits.*') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                ...
                                <a href="{{ route('referrals') }}" class="flex items-center px-4 py-2 text-sm font-semibold transition-colors {{ request()->routeIs('referrals') ? 'bg-orange-50 text-orange-600' : 'text-gray-700 hover:bg-orange-50 hover:text-orange-600' }}">
                                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                                    Referrals
                                </a>
                            </div>
                            ...
                            <!-- Menu Items -->
                            <div class="py-2">
                                <a href="{{ route('profile') }}" class="flex items-center px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-colors">
                                    <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                    Profile Settings
                                </a>
                                <a href="{{ route('transactions.index') }}" class="flex items-center px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-colors">
                            ...
                            <div class="border-t border-gray-100 pt-2">
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="flex items-center w-full px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 transition-colors">
