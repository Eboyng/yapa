<div class="min-h-screen bg-gradient-to-br from-orange-50 via-white to-purple-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        @include('components.wallet-balances', ['creditsBalance' => $this->creditsBalance, 'nairaBalance' => $this->nairaBalance, 'earningsBalance' => $this->earningsBalance])
        ...
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <div id="batchShareTab" class="p-4 sm:p-6 lg:p-8 tab-content hidden">
                ...
                <div class="border border-gray-200 rounded-2xl p-4 sm:p-6 mb-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-semibold text-gray-900">My Shared Batches</h3>
                        <button wire:click="toggleSharedBatchesList" 
                                class="text-sm text-purple-600 hover:text-purple-700 font-medium">
                            @if($showSharedBatchesList) Hide Details @else Show Details @endif
                        </button>
                    </div>
                    ...
                    @if($showSharedBatchesList)
                        ...
                        @else
                            @if(count($sharedBatches) > 0)
                                <div class="space-y-4">
                                    @foreach($sharedBatches as $sharedBatch)
                                        <div class="border border-gray-200 rounded-lg p-4 bg-gradient-to-r from-purple-50 to-blue-50">
                                            <div class="flex items-start justify-between mb-3">
                                                <div class="flex-1">
                                                    <h4 class="font-medium text-gray-900">{{ $sharedBatch['batch']['name'] }}</h4>
                                                    <p class="text-sm text-gray-600 mt-1">{{ $sharedBatch['batch']['description'] }}</p>
                                                    <div class="flex items-center mt-2 text-xs text-gray-500">
                                                        ...
                                                        Ends: {{ \Carbon\Carbon::parse($sharedBatch['batch']['end_date'])->format('M d, Y') }}
                                                    </div>
                                                </div>
                                                <div class="text-right">
                                                    @if($sharedBatch['reward_claimed'])
                                                    ...
                                                    @elseif($sharedBatch['can_claim_reward'])
                                            ...
                                            <div class="mb-3">
                                                <div class="flex items-center justify-between text-xs text-gray-600 mb-1">
                                                    <span>Progress to Reward ({{ $sharedBatch['new_members_count'] }}/10 new members)</span>
                                                    <span>{{ number_format($sharedBatch['progress_percentage'], 1) }}%</span>
                                                </div>
                                                <div class="w-full bg-gray-200 rounded-full h-2">
                                                    <div class="bg-gradient-to-r from-purple-500 to-blue-500 h-2 rounded-full transition-all duration-300" 
                                                         style="width: {{ $sharedBatch['progress_percentage'] }}%"></div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="grid grid-cols-4 gap-3">
                                                <div class="text-center">
                                                    <div class="flex items-center justify-center w-8 h-8 bg-green-100 rounded-lg mx-auto mb-1">
                                                        <svg class="w-4 h-4 text-green-600" fill="currentColor" viewBox="0 0 24 24">
                                                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.785"/>
                                                        </svg>
                                                    </div>
                                                    <p class="text-xs font-medium text-gray-900">{{ $sharedBatch['platform_stats']['whatsapp'] }}</p>
                                                    <p class="text-xs text-gray-500">WhatsApp</p>
                                                </div>
                                                <div class="text-center">
                                                    <div class="flex items-center justify-center w-8 h-8 bg-blue-100 rounded-lg mx-auto mb-1">
                                                        <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                                                            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                                                        </svg>
                                                    </div>
                                                    <p class="text-xs font-medium text-gray-900">{{ $sharedBatch['platform_stats']['facebook'] }}</p>
                                                ...
                                                <div class="text-center">
                                                    ...
                                                    <p class="text-xs font-medium text-gray-900">{{ $sharedBatch['platform_stats']['twitter'] }}</p>
                                                    <p class="text-xs text-gray-500">Twitter</p>
                                                </div>
                                                <div class="text-center">
                                                    <div class="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-lg mx-auto mb-1">
                                                        <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path>
                                                        </svg>
                                                    </div>
                                                    <p class="text-xs font-medium text-gray-900">{{ $sharedBatch['platform_stats']['copy_link'] }}</p>
                                                    <p class="text-xs text-gray-500">Copy Link</p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="mt-3 pt-3 border-t border-gray-200">
                                                <div class="flex items-center justify-between text-sm">
                                                    <span class="text-gray-600">Total Shares: <strong class="text-purple-600">{{ $sharedBatch['total_shares'] }}</strong></span>
                                                    <span class="text-gray-600">New Members: <strong class="text-green-600">{{ $sharedBatch['new_members_count'] }}</strong></span>
                                                    <span class="text-gray-600">Potential Reward: <strong class="text-orange-600">₦{{ number_format($sharedBatch['new_members_count'] * 10, 2) }}</strong></span>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <div class="text-center py-8">
                                    <svg class="w-12 h-12 text-gray-400 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path>
                                                    ...
                                                    <p class="text-gray-500">No shared batches yet</p>
                                                    <p class="text-sm text-gray-400">Start sharing batches below to track your progress!</p>
                                                </div>
                                            @endif
                                        @endif
                    @endif
